...
```
Sources/
  Application.swift    — everything: app config, theme, metadata, routes, views
Public/                — static assets (copied verbatim to output)
```

All pages and components live in `Application.swift` for this minimal template. Extract to separate files as the project grows.
...
## Application.swift Structure

```swift
@main
struct MyApp: Application {
    var metadata: SiteMetadata {
        SiteMetadata(title: "__NAME__", description: "…", baseURL: "https://example.com")
    }
...
struct HomePage: Page {
    var body: some View {
        Main {
            VStack {
                Heading(1) { "Hello" }.font(size: .fourXL, weight: .bold)
                Text { "Welcome" }.font(color: .muted)
            }
            .padding(8)
        }
    }
...
|------|------|
| `Stack { … }` | `<div>` |
| `VStack { … }` | `<div>` flex column |
| `HStack { … }` | `<div>` flex row |
| `ZStack { … }` | `<div>` position:relative |
| `Grid(columns:?) { … }` | `<div>` CSS Grid |
...
|------|------|
| `Link(to: String) { … }` | `<a>` |
| `NavLink(to: String) { … }` | `<a>` active-aware |
...
| View | HTML |
|------|------|
| `Image(_ src:, alt:, caption:?, loading:)` | `<img>` or `<figure>` |
| `Video(src:, controls:, autoplay:, loop:, muted:)` | `<video>` |
| `Audio(src:, controls:, autoplay:, loop:, muted:)` | `<audio>` |

### Forms

`Form(action:?, method:)` `Input(type:, name:, placeholder:?, label:?, required:)` `Label(for:)` `Option(value:)` `Fieldset` `Legend`
...
```swift
List(.unordered) { ListItem { … } }   // .unordered .ordered .decimal .alpha .none
DescriptionList { Term { … }; Description { … } }

Table { TableHeader { TableRow { TableCell(.header) { "H" } } }
        TableBody  { TableRow { TableCell { "D" } } } }
```
...
All return `ModifiedContent<Self, M>`. Combine params in one call.
...
.flex(direction:?, wrap:?, align:?, justify:?, gap:?, grow:?, shrink:?, basis:?, alignSelf:?, order:?)
.grid(columns:?, rows:?, gap:?, span:?, rowSpan:?, autoFlow:?, placeItems:?, alignSelf:?, justifySelf:?)
...
### Typography (prefer combined overload)
```swift
...
### Animation
```swift
.animate(_ animation: Animation, duration: AnimationDuration, easing: AnimationTiming = .easeOut)
...
### Semantic
```swift
Color.primary      // violet(600)     Color.accent       // emerald(400)
Color.surface      // white           Color.secondary    // slate(100)
Color.tertiary     // slate(50)       Color.text         // slate(900)
Color.muted        // slate(500)      Color.destructive  // rose(600)
...
Color(hsl: 263, 69, 58)
color.opacity(0.5)  color.lighten(0.1)  color.darken(0.1)  color.mix(other, by: 0.5)
```
...
Use `.erased()` to style the incoming `any View`:
...
ContentTheme(
    ...
    paragraph: { v in v.erased().font(leading: .relaxed).margin(y: .rem(0.75)) },
...
Built-in themes: `ContentTheme.default` (identity), `ContentTheme.blog`.

---

## String Literals as Views

`String` conforms to `View`. Use directly in `@ViewBuilder`:
