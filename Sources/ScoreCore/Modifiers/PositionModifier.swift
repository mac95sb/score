...
public struct PositionModifier: ThemeAwareModifier {
    ...
    public init(
        ...
        self.type = type; self.top = top; self.right = right
        self.bottom = bottom; self.left = left; self.inset = inset
        self.insetX = insetX; self.insetY = insetY
        self.zIndex = zIndex; self.condition = condition
    }
    ...
    public func declarations(theme: SiteTheme) -> [ConditionedDeclaration] {
        ...
        if let i = inset   { result.append(ConditionedDeclaration("inset",    i.css,      condition: condition)) }
        else {
            if let ix = insetX {
