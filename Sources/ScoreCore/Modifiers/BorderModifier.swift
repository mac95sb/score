...
/// A modifier that applies CSS border and border-radius properties to an element.
///
/// Use ``View/border(color:width:edge:style:radius:on:)`` and its overloads rather
...
/// ```
///
/// - SeeAlso: ``View/border(color:width:edge:style:radius:on:)``
public struct BorderModifier: ThemeAwareModifier {
    ...
    public func declarations(theme: SiteTheme) -> [ConditionedDeclaration] {
        ...
        // Border stroke — only emitted when stroke-related fields are set
        let hasStroke = color != nil || edge != nil || edges != nil
        if hasStroke {
        ...
        // Border radius
        if let r = radius {
            result.append(ConditionedDeclaration("border-radius", "\(theme.radii[r].cssStr)px", condition: condition))
...
/// Backward-compatible alias — prefer the unified `BorderModifier`.
public typealias BorderRadiusModifier = BorderModifier
